...
export default function Labeling() {
  ...
  const uploadExcel = async (file: File) => {
    ...
    try {
      ...
      if (shs.length > 1) {
        ...
        setUploadingExcel(false); return false;
      }
      finishUploadExcel(res.data);
    } catch { message.error('上传失败'); setUploadingExcel(false); }
    return false;
  };

  const finishUploadExcel = (data: any) => {
    setDatasources(prev => [...prev, data]);
    setSelectedDS(data.id); setColumns(data.columns || []);
  ...
  const confirmSheet = async () => {
    ...
    try {
      ...
      setUploadingExcel(false);
      if (selectedDS === pendingDsId) { setColumns(res.data.columns || []); }
      setSheets([]); setSelectedSheet(''); setPendingDsId(null);
      message.success(`已选择 Sheet: ${res.data.sheet_used}`);
    } catch { message.error('切换 Sheet 失败'); setUploadingExcel(false); }
  };
  ...
  const uploadWord = async (file: File) => {
    ...
    try {
      ...
      setPrompts(prev => [...prev, res.data]); setSelectedPrompt(res.data.id);
      message.success('已上传提示词');
  ...
  const startLivePoll = useCallback((jobId: number) => {
    ...
    livePollRef.current = setInterval(async () => {
      try {
        ...
        failCount = 0; setReconnecting(false);
  ...
  const startPoll = useCallback((jobId: number) => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        setJob(res.data); failCount = 0; setReconnecting(false);
        if (res.data.status === 'done' || res.data.status === 'failed') { stopPoll(); localStorage.removeItem(`active_job_${id}`); }
      } catch { failCount++; if (failCount >= 3) setReconnecting(true); }
    }, 2000) as unknown as number;
  }, [id, stopPoll]);

  const refreshHasKey = useCallback(() => {
    api.get('/projects/ai-config').then(r => { setHasKey(r.data.has_key); if (r.data.model_name) setCurrentModel(r.data.model_name); });
  }, []);
  ...
  const reconnectToJob = useCallback(async (jobId: number) => {
    try {
      const res = await api.get(`/labeling/job/${jobId}`);
      setJob(res.data);
      if (res.data.status === 'running' || res.data.status === 'pending') { startPoll(jobId); startLivePoll(jobId); }
      else if (res.data.status === 'paused' || res.data.status === 'done') { fetchRowsOnce(jobId); }
  ...
  const checkExistingJob = async () => {
    try {
      ...
      if (Array.isArray(res.data) && res.data.length > 0) {
        const active = res.data.find((j: any) => j.status === 'running' || j.status === 'paused' || j.status === 'pending');
  ...
  const doStartLabeling = async (forceOverride = false) => {
    if (!selectedDS || !selectedPrompt || !targetField) { message.warning('请选择数据文件、提示词文档和分析字段'); return; }
    try {
  ...
  const resumeJob = async () => { if (!job) return;
    try { await api.post(`/labeling/${job.id}/resume`); setJob({ ...job, status: 'running' });
      startPoll(job.id); startLivePoll(job.id); message.info('已继续'); }
        ...
        <Space direction="vertical" style={{ width: '100%' }} size="middle">
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <Space.Compact style={{ width: '100%' }}>
                <Upload beforeUpload={uploadExcel} showUploadList={false} accept=".xlsx,.xls">
                  <Button icon={<UploadOutlined />} loading={uploadingExcel && !pendingDsId}>上传</Button>
                </Upload>
              ...
              {sheets.length > 1 && (
                <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <Button size="small" type="primary" loading={uploadingExcel} onClick={confirmSheet}>确认</Button>
                  <Button size="small" onClick={() => { setSheets([]); setPendingDsId(null); }}>取消</Button>
                </div>
              )}
              {/* File list with delete */}
              {datasources.length > 0 && (
                <div style={{ marginTop: 4, maxHeight: 100, overflow: 'auto', border: '1px solid #f0f0f0', borderRadius: 4, padding: 2 }}>
                  {datasources.map(ds => (
                    <div key={ds.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 11, padding: '3px 6px', borderBottom: '1px solid #fafafa' }}>
                      <span style={{ color: '#444', cursor: 'pointer' }} onClick={() => onSelectDS(ds.id)}>{ds.filename} ({ds.row_count}行)</span>
                      <Popconfirm title="删除此文件？" onConfirm={async () => {
                        await api.delete(`/upload/datasource/${ds.id}`);
                        setDatasources(prev => prev.filter(x => x.id !== ds.id));
                        if (selectedDS === ds.id) setSelectedDS(null);
                        message.success('已删除');
                      }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {prompts.length > 0 && (
                <div style={{ marginTop: 4, maxHeight: 80, overflow: 'auto', border: '1px solid #f0f0f0', borderRadius: 4, padding: 2 }}>
                  {prompts.map(p => (
                    <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 11, padding: '3px 6px' }}>
                      <span style={{ color: '#444', cursor: 'pointer' }} onClick={() => setSelectedPrompt(p.id)}>{p.filename}</span>
                      <Popconfirm title="删除此文件？" onConfirm={async () => {
                        await api.delete(`/upload/prompt/${p.id}`);
                        setPrompts(prev => prev.filter(x => x.id !== p.id));
                        if (selectedPrompt === p.id) setSelectedPrompt(null);
                        message.success('已删除');
                      }}>
                        <Button type="link" size="small" danger icon={<DeleteOutlined />} style={{ padding: 0 }} />
                      </Popconfirm>
          ...
          <Space>
            {!isRunning && !isPaused && (
              <>
                <div><label style={{ marginRight: 6 }}>并发：</label>
                  <Select size="small" style={{ width: 70 }} value={concurrency} onChange={setConcurrency}
                    options={[1, 2, 3, 5, 8].map(n => ({ label: String(n), value: n }))} />
                </div>
                <Button type="primary" icon={<PlayCircleOutlined />} onClick={startLabeling} disabled={!hasKey}>启动 AI 标注</Button>
              </>
            )}
            {(isRunning || isPaused) && (
              <>
